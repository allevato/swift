...
std::string declUSR(const Decl *D) {
  if (!D) return "";
//  if (auto VD = dyn_cast<ValueDecl>(D); VD && VD->getRawLocalDiscriminator() == ValueDecl::InvalidDiscriminator) return "";
...
std::string jsonStringForAvailabilitySpecKind(AvailabilitySpecKind kind) {
  switch (kind) {
  case AvailabilitySpecKind::PlatformVersionConstraint: return "platformVersion";
  case AvailabilitySpecKind::OtherPlatform: return "otherPlatform";
  case AvailabilitySpecKind::LanguageVersionConstraint: return "languageVersion";
  case AvailabilitySpecKind::PackageDescriptionVersionConstraint: return "packageDescription";
  }
}
...
std::string jsonStringForGenericTypeParamKind(GenericTypeParamKind kind) {
  switch (kind) {
  case GenericTypeParamKind::Type: return "type";
  case GenericTypeParamKind::Pack: return "pack";
  case GenericTypeParamKind::Value: return "value";
  }
}
...
std::string jsonStringForKeyPathExprComponentKind(KeyPathExpr::Component::Kind kind) {
  switch (kind) {
  case KeyPathExpr::Component::Kind::Invalid: return "invalid";
  case KeyPathExpr::Component::Kind::UnresolvedProperty: return "unresolvedProperty";
  case KeyPathExpr::Component::Kind::UnresolvedSubscript: return "unresolvedSubscript";
  case KeyPathExpr::Component::Kind::Property: return "property";
  case KeyPathExpr::Component::Kind::Subscript: return "subscript";
  case KeyPathExpr::Component::Kind::OptionalForce: return "optionalForce";
  case KeyPathExpr::Component::Kind::OptionalChain: return "optionalChain";
  case KeyPathExpr::Component::Kind::OptionalWrap: return "optionalWrap";
  case KeyPathExpr::Component::Kind::Identity: return "identity";
  case KeyPathExpr::Component::Kind::TupleElement: return "tupleElement";
  case KeyPathExpr::Component::Kind::DictionaryKey: return "dictionaryKey";
  case KeyPathExpr::Component::Kind::CodeCompletion: return "codeCompletion";
  }
}

std::string jsonStringForLayoutConstraintKind(LayoutConstraintKind kind) {
  switch (kind) {
  case LayoutConstraintKind::UnknownLayout: return "unknown";
  case LayoutConstraintKind::TrivialOfExactSize: return "trivialOfExactSize";
  case LayoutConstraintKind::TrivialOfAtMostSize: return "trivialOfAtMostSize";
  case LayoutConstraintKind::Trivial: return "trivial";
  case LayoutConstraintKind::Class: return "class";
  case LayoutConstraintKind::NativeClass: return "nativeClass";
  case LayoutConstraintKind::RefCountedObject: return "refCountedObject";
  case LayoutConstraintKind::NativeRefCountedObject: return "nativeRefCountedObject";
  case LayoutConstraintKind::BridgeObject: return "bridgeObject";
  case LayoutConstraintKind::TrivialStride: return "trivialStride";
  }
}

std::string jsonStringForRequirementKind(RequirementKind kind) {
  switch (kind) {
  case RequirementKind::Conformance: return "conformance";
  case RequirementKind::Superclass: return "superclass";
  case RequirementKind::SameType: return "sameType";
  case RequirementKind::Layout: return "layout";
  case RequirementKind::SameShape: return "sameShape";
  }
}

std::string jsonStringForStmtConditionKind(StmtConditionElement::ConditionKind kind) {
  switch (kind) {
  case StmtConditionElement::CK_Boolean: return "boolean";
  case StmtConditionElement::CK_PatternBinding: return "pattern";
  case StmtConditionElement::CK_Availability: return "availability";
  case StmtConditionElement::CK_HasSymbol: return "hasSymbol";
  }
}

StringRef typeReprKindName(TypeReprKind kind) {
  switch (kind) {
#define TYPEREPR(Id, Parent) case TypeReprKind::Id: return #Id;
#include "swift/AST/TypeReprNodes.def"
  }
  llvm_unreachable("bad TypeReprKind");
}
...
public:
  JSONASTVisitor(llvm::raw_ostream &os, ASTContext &ctx) : json(os, /*indent=*/ 0), ctx(ctx) {}
  ...
  void dumpAttributes(Decl *D) {
    ...
    attributeBool("isImplementationOnly", attrs.hasAttribute<ImplementationOnlyAttr>());
    attributeBool("isLazy", attrs.hasAttribute<LazyAttr>());
    attributeBool("isKnownToBeLocal", attrs.hasAttribute<KnownToBeLocalAttr>());
    attributeBool("isObjC", attrs.hasAttribute<ObjCAttr>());
  ...
  void visitValueDecl(ValueDecl *D) {
    ...
    attributeArray("satisfiedProtocolRequirementUSRs", declUSRs(D->getSatisfiedProtocolRequirements()));
  }
  ...
  void visitTypeDecl(TypeDecl *D) {
    ...
    attributeString("declaredInterfaceTypeUSR", typeUSR(D->getDeclaredInterfaceType()));
  ...
  void visitOpaqueTypeDecl(OpaqueTypeDecl *D) {
    ...
    attribute("uniqueUnderlyingTypeSubstitutions", D->getUniqueUnderlyingTypeSubstitutions());
  }
  ...
  void visitGenericTypeParamDecl(GenericTypeParamDecl *D) {
    ...
    attributeString("paramKind", jsonStringForGenericTypeParamKind(D->getParamKind()));
  ...
  void visitAbstractStorageDecl(AbstractStorageDecl *D) {
    visitValueDecl(D);
    attributeString("staticSpelling", jsonStringForStaticSpellingKind(D->getCorrectStaticSpelling()));
    ...
    if (bool isMutable = impl.supportsMutation()) {
      ...
      attributeString("writeImpl", jsonStringForWriteImplKind(impl.getWriteImpl()));
      attributeString("readWriteImpl", jsonStringForReadWriteImplKind(impl.getReadWriteImpl()));
    }
  }
  ...
  void visitBreakStmt(BreakStmt *S) {
    attribute("target", S->getTargetName());
  }
  ...
  void visitExpr(Expr *E) {
    attribute("typeUSR", typeUSR(E->getType()));
  }
  ...
  void visitStringLiteralExpr(StringLiteralExpr *E) {
    ...
    attributeBool("isSingleExtendedGraphemeCluster", E->isSingleExtendedGraphemeCluster());
  ...
  void visitMagicIdentifierLiteralExpr(MagicIdentifierLiteralExpr *E) {
    visitBuiltinLiteralExpr(E);
    attributeString("magicIdentifierKind", MagicIdentifierLiteralExpr::getKindString(E->getKind()));
  }
  ...
  void visitRegexLiteralExpr(RegexLiteralExpr *E) {
    visitLiteralExpr(E);
    attributeString("regexText", E->getRegexText());
  ...
  void visitDotSyntaxBaseIgnored(DotSyntaxBaseIgnoredExpr *E) {
    visitExpr(E);
  ...
  void visitOverloadSetRefExpr(OverloadSetRefExpr *E) {
    visitExpr(E);
    if (!E->getDecls().empty()) {
      json.attributeArray("declUSRs", [&](){
        for (const auto *D : E->getDecls()) {
          json.value(declUSR(D));
        }
      });
    }
  }
  ...
  void visitCaptureListExpr(CaptureListExpr *E) {
    visitExpr(E);
    attributeArray("captures", E->getCaptureList());
  ...
  void visitClosureExpr(ClosureExpr *E) {
    ...
    attributeString("explicitThrownTypeUSR", typeUSR(E->getExplicitThrownType()));
  ...
  void visitPropertyWrapperValuePlaceholderExpr(PropertyWrapperValuePlaceholderExpr *E) {}
  ...
  void visitMacroExpansionExpr(MacroExpansionExpr *E) {
    visitExpr(E);
    attribute("macroName", E->getMacroName());
  ...
  void visitExprPattern(ExprPattern *P) {
    ...
    attributeString("ownership", getOwnershipSpelling(P->getCachedMatchOperandOwnership()));
  }
...
private:
  ...
  void value(ASTNode node) {
    writeNode(node, [&](){
      if (auto D = node.dyn_cast<Decl *>()) {
        ...
        visit(E);
      } else if (auto T = node.dyn_cast<TypeRepr *>()) {
        visit(T);
      }
    });
  }
  ...
  void value(const AvailabilitySpec *spec) {
    json.object([&](){
      attributeString("kind", jsonStringForAvailabilitySpecKind(spec->getKind()));
      ...
      } else if (auto S = dyn_cast<PlatformAgnosticVersionConstraintAvailabilitySpec>(spec)) {
        attributeString("version", S->getVersion().getAsString());
  ...
  void value(const CaptureListEntry &entry) {
    json.object([&](){
      attribute("patternBinding", entry.PBD);
    });
  }
  ...
  template <typename T, typename std::enable_if<std::is_same<T, ConcreteDeclRef>::value>::type* = nullptr>
  ...
  template <typename T, typename std::enable_if<std::is_same<T, DeclBaseName>::value>::type* = nullptr>
  ...
  template <typename T, typename std::enable_if<std::is_same<T, DeclName>::value>::type* = nullptr>
  ...
  template <typename T, typename std::enable_if<std::is_same<T, DeclNameRef>::value>::type* = nullptr>
  void value(const T &ref) {
    value(ref.getFullName());
  }

  template <typename T, typename std::enable_if<std::is_same<T, Identifier>::value>::type* = nullptr>
  ...
  void value(KeyPathExpr::Component component) {
    json.object([&](){
      attributeString("kind", jsonStringForKeyPathExprComponentKind(component.getKind()));
      ...
      case KeyPathExpr::Component::Kind::Subscript:
        attribute("resolvedDeclRef", component.getDeclRef());
        attributeArray("args", *component.getSubscriptArgs());
        attributeArray("hashableConformances", component.getSubscriptIndexHashableConformances());
        break;
  ...
  void value(const PoundHasSymbolInfo *info) {
    json.object([&](){
      attribute("symbolExpr", info->getSymbolExpr());
    });
  }
  ...
  template <typename T, typename std::enable_if<std::is_same<T, StmtConditionElement>::value>::type* = nullptr>
  void value(const T &element) {
    json.object([&](){
      attributeString("kind", jsonStringForStmtConditionKind(element.getKind()));
  ...
  void value(const RequirementRepr &req) {
    json.object([&](){
      switch (req.getKind()) {
      case RequirementReprKind::LayoutConstraint:
        attribute("subject", req.getSubjectRepr());
        attribute("layout", req.getLayoutConstraint());
        break;

      case RequirementReprKind::TypeConstraint:
        attribute("subject", req.getSubjectRepr());
        attribute("constraint", req.getConstraintRepr());
        break;

      case RequirementReprKind::SameType:
        attribute("first", req.getFirstTypeRepr());
        attribute("second", req.getSecondTypeRepr());
        break;
      }
    });
  }
  ...
  /// the current object.
  template <typename T, typename std::enable_if<!std::is_constructible<bool, T>::value>::type* = nullptr>
  ...
  void attributeArray(StringRef k, const ArgumentList *args) {
    if (args == nullptr || args->empty()) return;
  ...
  void writeKindAttribute(ASTNode node) {
    ...
    } else if (TypeRepr *T = node.dyn_cast<TypeRepr *>()) {
      attributeString("kind", std::string(typeReprKindName(T->getKind())) + "TypeRepr");
    }
  }
