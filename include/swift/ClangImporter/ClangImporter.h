...
public:
  ...
  /// module map into the replica and emits a PCM file of its headers. Delegates
  /// to clang for everything except construction of the replica.
  bool emitPrecompiledModule(StringRef moduleMapPath,
                             StringRef outputPCMPath);

  /// Makes a temporary replica of the ClangImporter's CompilerInstance and
  /// dumps information about a PCM file (assumed to be generated by
  /// -generate-pcm or in the Swift module cache). Delegates to clang for
  /// everything except construction of the replica.
  bool dumpPrecompiledModuleInfo(StringRef pcmPath);
